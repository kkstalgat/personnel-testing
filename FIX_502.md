# 🔧 Исправление ошибки 502 Bad Gateway

## 🚨 Быстрое решение

### 1. Проверка статуса Gunicorn

```bash
# Проверьте, запущен ли сервис
sudo systemctl status personnel_testing

# Если не запущен, запустите
sudo systemctl start personnel_testing

# Проверьте статус снова
sudo systemctl status personnel_testing
```

### 2. Проверка сокета

```bash
# Проверьте, существует ли сокет
ls -la /var/www/personnel_testing/personnel_testing.sock

# Проверьте права доступа
# Сокет должен принадлежать пользователю, который запускает Gunicorn
# И Nginx должен иметь права на чтение
```

### 3. Проверка прав доступа

```bash
# Узнайте, какой пользователь запускает Gunicorn
ps aux | grep gunicorn | head -1

# Узнайте, какой пользователь запускает Nginx
ps aux | grep nginx | head -1

# Исправьте права на сокет (замените deploy на вашего пользователя)
sudo chown deploy:www-data /var/www/personnel_testing/personnel_testing.sock
sudo chmod 660 /var/www/personnel_testing/personnel_testing.sock
```

### 4. Проверка логов

```bash
# Логи Nginx
sudo tail -n 50 /var/log/nginx/error.log

# Логи Gunicorn
sudo tail -n 50 /var/www/personnel_testing/logs/error.log

# Логи systemd
sudo journalctl -u personnel_testing -n 50
```

---

## 📋 Пошаговая диагностика

### Шаг 1: Проверка статуса сервиса

```bash
sudo systemctl status personnel_testing --no-pager -l
```

**Если сервис не запущен:**
```bash
sudo systemctl start personnel_testing
sudo systemctl enable personnel_testing  # Для автозапуска
```

**Если сервис падает:**
```bash
# Проверьте логи
sudo journalctl -u personnel_testing -n 100

# Проверьте конфигурацию
sudo /var/www/personnel_testing/venv/bin/gunicorn --check-config \
    --config /var/www/personnel_testing/gunicorn_config.py \
    personnel_testing.wsgi:application
```

### Шаг 2: Проверка сокета

```bash
# Проверьте существование сокета
ls -la /var/www/personnel_testing/personnel_testing.sock

# Если сокета нет, перезапустите Gunicorn
sudo systemctl restart personnel_testing

# Подождите 2-3 секунды и проверьте снова
ls -la /var/www/personnel_testing/personnel_testing.sock
```

### Шаг 3: Проверка прав доступа

```bash
# Узнайте пользователя Gunicorn из systemd service
sudo cat /etc/systemd/system/personnel_testing.service | grep User

# Узнайте пользователя Nginx
ps aux | grep nginx | grep master | awk '{print $1}'

# Обычно это www-data или nginx
# Проверьте права на сокет
ls -la /var/www/personnel_testing/personnel_testing.sock

# Исправьте права (пример для пользователя deploy и группы www-data)
sudo chown deploy:www-data /var/www/personnel_testing/personnel_testing.sock
sudo chmod 660 /var/www/personnel_testing/personnel_testing.sock
```

### Шаг 4: Проверка конфигурации Nginx

```bash
# Проверьте конфигурацию Nginx
sudo nginx -t

# Проверьте, правильно ли указан путь к сокету
sudo grep -A 5 "proxy_pass" /etc/nginx/sites-available/personnel_testing

# Должно быть что-то вроде:
# proxy_pass http://unix:/var/www/personnel_testing/personnel_testing.sock;
```

### Шаг 5: Перезапуск сервисов

```bash
# Перезапустите Gunicorn
sudo systemctl restart personnel_testing

# Подождите 2-3 секунды
sleep 3

# Проверьте статус
sudo systemctl status personnel_testing

# Перезагрузите Nginx
sudo systemctl reload nginx
# или
sudo systemctl restart nginx
```

---

## 🔍 Типичные причины 502

### 1. Gunicorn не запущен

**Симптомы:**
- `systemctl status` показывает `inactive` или `failed`
- Сокет не существует

**Решение:**
```bash
sudo systemctl start personnel_testing
sudo systemctl status personnel_testing
```

### 2. Проблемы с правами доступа

**Симптомы:**
- В логах Nginx: `connect() to unix:/var/www/personnel_testing/personnel_testing.sock failed (13: Permission denied)`
- Сокет существует, но Nginx не может подключиться

**Решение:**
```bash
# Узнайте пользователей
GUNICORN_USER=$(ps aux | grep "[g]unicorn" | head -1 | awk '{print $1}')
NGINX_USER=$(ps aux | grep "[n]ginx.*master" | head -1 | awk '{print $1}')

# Если NGINX_USER пустой, используйте www-data (стандартный пользователь Nginx)
if [ -z "$NGINX_USER" ]; then
    NGINX_USER="www-data"
fi

# Исправьте права на сокет
sudo chown $GUNICORN_USER:$NGINX_USER /var/www/personnel_testing/personnel_testing.sock
sudo chmod 660 /var/www/personnel_testing/personnel_testing.sock

# Перезагрузите Nginx
sudo systemctl reload nginx
```

**Для постоянного решения:**

1. Создайте скрипт для исправления прав (скрипт `fix_socket_permissions.sh` уже создан в проекте):
```bash
# Скопируйте скрипт на сервер и сделайте исполняемым
sudo cp /var/www/personnel_testing/fix_socket_permissions.sh /usr/local/bin/
sudo chmod +x /usr/local/bin/fix_socket_permissions.sh
```

2. Добавьте в systemd service файл `/etc/systemd/system/personnel_testing.service`:
```ini
[Service]
...
ExecStartPost=/usr/local/bin/fix_socket_permissions.sh
```

3. После этого выполните:
```bash
sudo systemctl daemon-reload
sudo systemctl restart personnel_testing
```

**Альтернативный вариант** (если скрипт не работает):
```ini
[Service]
...
ExecStartPost=/bin/bash -c '/bin/sleep 2; /bin/chown ubuntu:www-data /var/www/personnel_testing/personnel_testing.sock; /bin/chmod 660 /var/www/personnel_testing/personnel_testing.sock'
```

**Важно:** Используйте точку с запятой `;` вместо `&&`, так как в systemd service `&&` может работать некорректно.

**Или еще проще** - используйте systemd timer или создайте отдельный скрипт с полными путями:
```ini
[Service]
...
ExecStartPost=/usr/local/bin/fix_socket_permissions.sh
```

И в скрипте используйте полные пути:
```bash
#!/bin/bash
SOCKET_PATH="/var/www/personnel_testing/personnel_testing.sock"
/bin/sleep 2
if [ -S "$SOCKET_PATH" ]; then
    /bin/chown ubuntu:www-data "$SOCKET_PATH"
    /bin/chmod 660 "$SOCKET_PATH"
fi
exit 0
```

### 3. Сокет не создается

**Симптомы:**
- Сокет отсутствует после запуска Gunicorn
- В логах: ошибки создания сокета

**Решение:**
```bash
# Проверьте права на директорию
ls -la /var/www/personnel_testing/ | grep personnel_testing.sock

# Проверьте, может ли пользователь создавать файлы в директории
sudo -u deploy touch /var/www/personnel_testing/test_socket
sudo rm /var/www/personnel_testing/test_socket

# Если не может, исправьте права
sudo chown -R deploy:deploy /var/www/personnel_testing
```

### 4. Неправильный путь к сокету в Nginx

**Симптомы:**
- В логах Nginx: `no such file or directory`

**Решение:**
```bash
# Проверьте конфигурацию Nginx
sudo grep "proxy_pass" /etc/nginx/sites-available/personnel_testing

# Убедитесь, что путь совпадает с gunicorn_config.py
sudo grep "bind" /var/www/personnel_testing/gunicorn_config.py
```

### 5. Gunicorn падает сразу после запуска

**Симптомы:**
- Сервис запускается, но сразу падает
- В логах systemd видны ошибки

**Решение:**
```bash
# Проверьте логи
sudo journalctl -u personnel_testing -n 100

# Проверьте конфигурацию
sudo /var/www/personnel_testing/venv/bin/gunicorn --check-config \
    --config /var/www/personnel_testing/gunicorn_config.py \
    personnel_testing.wsgi:application

# Проверьте переменные окружения
sudo cat /etc/systemd/system/personnel_testing.service | grep Environment
```

---

## 🛠️ Полная диагностика одной командой

```bash
echo "=== Статус сервиса ===" && \
sudo systemctl status personnel_testing --no-pager -l && \
echo -e "\n=== Проверка сокета ===" && \
ls -la /var/www/personnel_testing/personnel_testing.sock 2>&1 && \
echo -e "\n=== Процессы Gunicorn ===" && \
ps aux | grep gunicorn | grep -v grep && \
echo -e "\n=== Последние ошибки Nginx ===" && \
sudo tail -n 20 /var/log/nginx/error.log && \
echo -e "\n=== Последние ошибки Gunicorn ===" && \
sudo tail -n 20 /var/www/personnel_testing/logs/error.log
```

---

## ✅ Быстрое исправление (если ничего не помогло)

```bash
# 1. Остановите сервисы
sudo systemctl stop personnel_testing
sudo systemctl stop nginx

# 2. Удалите старый сокет (если есть)
sudo rm -f /var/www/personnel_testing/personnel_testing.sock

# 3. Проверьте права на директорию
sudo chown -R deploy:deploy /var/www/personnel_testing
sudo chmod 755 /var/www/personnel_testing

# 4. Запустите Gunicorn
sudo systemctl start personnel_testing
sleep 3

# 5. Проверьте сокет
ls -la /var/www/personnel_testing/personnel_testing.sock

# 6. Исправьте права на сокет
sudo chown deploy:www-data /var/www/personnel_testing/personnel_testing.sock
sudo chmod 660 /var/www/personnel_testing/personnel_testing.sock

# 7. Запустите Nginx
sudo systemctl start nginx

# 8. Проверьте статус
sudo systemctl status personnel_testing
sudo systemctl status nginx
```

---

## 📞 Если проблема не решена

1. Проверьте все логи:
   ```bash
   sudo journalctl -u personnel_testing -n 100
   sudo tail -n 100 /var/log/nginx/error.log
   sudo tail -n 100 /var/www/personnel_testing/logs/error.log
   ```

2. Проверьте конфигурацию:
   ```bash
   sudo nginx -t
   sudo /var/www/personnel_testing/venv/bin/gunicorn --check-config \
       --config /var/www/personnel_testing/gunicorn_config.py \
       personnel_testing.wsgi:application
   ```

3. Попробуйте запустить Gunicorn вручную для отладки:
   ```bash
   cd /var/www/personnel_testing
   source venv/bin/activate
   gunicorn --config gunicorn_config.py personnel_testing.wsgi:application
   ```
